...
export const acrobatics = new Skill({
	...
	specialties: {
		...
		jump: new Specialty({
			name: `Jump`,
			description: [`Running Jump [Speed]. Vertical [Speed x 3"].`,]
		})
	}
})
...
export const larceny = new Skill({
	...
	specialties: {
		mechanical: new Specialty({
			name: `Mechanical`,
			description: [`(d6 rounds) Activate or deactivate Locks, Traps, Bombs, and similar mechanisms. # by item.`,]
		}),
		trick: new Specialty({
			name: `Trick`,
...
export const ranged = new Skill({
	...
	specialties: {
		shoot: new Specialty({
			name: `Shoot`,
			description: [`Roll vs [Dodge or Block (with a Shield)].`,]
		}),
		throw: new Specialty({
			name: `Throw`,
			description: [`Roll vs [Dodge or Block]. Range is [Constitution x 3yds]`,]
		})
	}
})
...
export const stealth = new Skill({
	...
	specialties: {
		...
		sneak: new Specialty({
			name: `Sneak`,
			description: [`Move [Speed / 2] while Concealed.`,]
		})
	}
})
...
export const medicine = new Skill({
	name: `Medicine`,
	description: [`Diagnosing and treating wounds.`,],
	parent: `Brains`,
	difficulty: 'Wounds',
	specialties: {
		firstaid: new Specialty({
			name: `First-Aid`,
			description: [`Stop Bleeding. Takes 1d6 rounds.`,]
		}),
		surgery: new Specialty({
			name: `Surgery`,
			description: [`Heal 1 Wound. Cannot be performed again on the same patient until they are fully healed. d6 Damage on a Botch.`,]
		})
	}
})
...
export const science = new Skill({
	...
	specialties: {
		chemistry: new Specialty({
			name: `Chemistry`,
			description: [`(# x 10mins) Use [d6 + # Chemicals].`,]
		}),
...
export const athletics = new Skill({
	...
	specialties: {
		climb: new Specialty({
			name: `Climb`,
			description: [`Move along vertical surfaces at [Speed / 2].`,]
		}),
		swim: new Specialty({
			name: `Swim`,
...
export const build = new Skill({
	...
	specialties: {
		customize: new Specialty({
			name: `Customize`,
			description: [`(#hrs) 3 per item. Each must be unique. Weapons: +1 Ranged Attack, +1 Melee Damage, or a new Attribute. Armor: +1 Damage Resistance or a new Attribute.`,]
		}),
		repair: new Specialty({
			name: `Repair`,
			description: [`(#hrs) Fix broken item. +1 with same Parts.`,]
		})
	}
})
...
export const melee = new Skill({
	...
	specialties: {
		...
		strike: new Specialty({
			name: `Strike`,
			description: [`Roll vs [Defense]. Damage = [weapon Damage + Success].`,]
		})
	}
})
...
export const leadership = new Skill({
	name: `Leadership`,
	description: [`Directing the efforts of others. Modifiers from multiple uses of the same Leadership Specialty do not stack.`,],
	parent: `Demeanor`,
	difficulty: 'Demeanor',
	specialties: {
		...
		intimidate: new Specialty({
			name: `Intimidate`,
			description: [`Roll vs [total target(s) Demeanor]. Target(s) take a penalty = [your Demeanor] to any roll except one you choose.`,]
		})
	}
})
...
export const socialize = new Skill({
	...
	specialties: {
		...
		therapy: new Specialty({
			name: `Therapy`,
			description: [`Heal 1 Trauma. Cannot be performed again on the same day. d6 Damage on a Botch.`,]
		})
	}
})
